...
public class UserRegUI {
    ...
    public void testLoginButtonClick() {
        ...
        // Check if User_Login activity is displayed (this requires you to have some identifiable view in User_Login)
        // Assuming User_Login activity has a view with id userLoginView
    ...
    public void testInvalidPhoneValidation() {
        // Enter invalid phone and valid values for other fields
        Espresso.onView(ViewMatchers.withId(R.id.reg_name)).perform(ViewActions.typeText("Test User"), ViewActions.closeSoftKeyboard());
        Espresso.onView(ViewMatchers.withId(R.id.reg_email)).perform(ViewActions.typeText("test@example.com"), ViewActions.closeSoftKeyboard());
        Espresso.onView(ViewMatchers.withId(R.id.reg_pin)).perform(ViewActions.typeText("123456"), ViewActions.closeSoftKeyboard());
        Espresso.onView(ViewMatchers.withId(R.id.reg_phone)).perform(ViewActions.typeText("1234"), ViewActions.closeSoftKeyboard());

        // Click register button
        Espresso.onView(ViewMatchers.withId(R.id.btnreg)).perform(ViewActions.click());

        // Check if phone error message is displayed
        Espresso.onView(ViewMatchers.withId(R.id.reg_phone)).check(ViewAssertions.matches(ViewMatchers.hasErrorText("Enter valid phone number")));
    }
    ...
    public void testValidRegistration() {
        ...
        // Assuming User_Dash activity has a view with id userDashView
        // Check if User_Dash activity is displayed
        // Espresso.onView(ViewMatchers.withId(R.id.userDashView)).check(ViewAssertions.matches(ViewMatchers.isDisplayed()));
    }
}
